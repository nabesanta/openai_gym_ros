...
class ActionValue(Chain):
    def __init__(self, n_history, n_act):
        super(ActionValue, self).__init__(
            ...
            l2=L.Convolution2D(32, 64, ksize=4, stride=2, nobias=False),
            l3=L.Convolution2D(64, 64, ksize=3, stride=1, nobias=False),
            l4=L.Linear(3136, 512),#),
            q_value=L.Linear(512, n_act,
...
class DQN:
    ...
    def __init__(self, n_history, n_act):
        ...
        self.replay_buffer = [np.zeros((self.data_size, hs, ims, ims), dtype=np.uint8),
                  ...
                  np.zeros((self.data_size, 1), dtype=bool)]
    ...
    def experience_replay(self, time):
        ...
        if self.initial_exploration < time:
            ...
            episode_end_replay = np.ndarray(shape=(rs, 1), dtype=bool)
            for i in range(self.replay_size):
...
class DQN_Agent:  # RL-glue Process
    ...
    def save(self):
        ensure_dir('network')  # ディレクトリの存在を確認し、なければ作成する
